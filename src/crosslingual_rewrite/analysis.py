...
def write_report(
    ...
    *,
    ...
    lines.append(
        ...
        "all available runs against the same fixed retriever and corpus. "
        "`machine_translate` is the automatic translation baseline; `translate` "
        "uses the gold target query and should be treated as an upper bound."
    )
    lines.append("")
    ...
    lines.append(
        "* Mock-model runs reuse the gold `target_query` when available, so supervised, "
        "retrieval-aware, and machine-translate smoke numbers may closely track the "
        "gold-query upper bound."
    )
    lines.append("")
